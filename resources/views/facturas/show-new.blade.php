...
<div x-data="facturaDetalle()" class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        {{-- Barra de acciones --}}
        <div class="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-200 dark:border-slate-700 flex flex-wrap items-center gap-3">
            @if($factura->pdf_path)
                <x-button variant="primary" size="md" @click="abrirVisorPDF()">
                ...
                <x-button variant="outline" size="md" href="{{ route('facturas.download-pdf', $factura) }}">
                    <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @else
                <span class="inline-flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 px-3 py-2 bg-gray-100 dark:bg-slate-700 rounded-lg">
            ...
            <x-button variant="secondary" size="md" href="{{ route('facturas.edit', $factura) }}">
                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <x-button variant="ghost" size="md" href="{{ route('facturas.index') }}">
                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <x-card title="Historial de Cambios">
                <x-slot:headerActions>
                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ $historial->count() }} registro(s)</span>
                </x-slot:headerActions>

                <div class="space-y-4">
                    @foreach($historial as $log)
                        <div class="flex gap-4 pb-4 {{ !$loop->last ? 'border-b border-gray-200 dark:border-slate-700' : '' }}">
                            {{-- Icono según acción --}}
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($log->old_values || $log->new_values)
                                    <button type="button"
                                            @click="toggleHistorialDetalle({{ $log->id }})"
                                            class="text-sm text-blue-600 dark:text-blue-400 hover:underline mt-1">
                                        Ver detalles
                                    </button>
                                    <div x-show="historialAbierto === {{ $log->id }}" x-collapse class="mt-2">
                                        <div class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-3 text-xs font-mono overflow-x-auto">
                                            @if($log->old_values)
                                                <p class="text-red-600 dark:text-red-400 mb-1">- {{ json_encode($log->old_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</p>
                                            @endif
                                            @if($log->new_values)
                                                <p class="text-green-600 dark:text-green-400">+ {{ json_encode($log->new_values, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                @endif
        ...
        <div class="space-y-6">
            {{-- Resumen Financiero --}}
            <x-card title="Resumen Financiero" class="sticky top-4">
                <div class="space-y-3">
