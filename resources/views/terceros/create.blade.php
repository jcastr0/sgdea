...
<div class="form-container">
    <div class="form-header">
        <h1>{{ isset($tercero) ? 'Editar Tercero' : 'Crear Nuevo Tercero' }}</h1>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="modal-actions">
                        <a href="{{ route('terceros.show', $duplicados[0]['id']) }}" class="btn btn-outline-primary">
                            Ver Duplicado
                        </a>
                        <form method="POST" action="{{ isset($tercero) ? route('terceros.update', $tercero) : route('terceros.store') }}" style="display: inline;">
                            @csrf
                            @method(isset($tercero) ? 'PUT' : 'POST')
                            @foreach ($tercero ?? [] as $key => $value)
                                <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                            @endforeach
                            <input type="hidden" name="confirmar_{{ isset($tercero) ? 'actualizar' : 'crear' }}" value="1">
    ...
    <form
        method="POST"
        action="{{ isset($tercero) ? route('terceros.update', $tercero) : route('terceros.store') }}"
        class="form-card"
        id="terceroForm"
    >
        @csrf
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-primary btn-lg">
                {{ isset($tercero) ? 'Actualizar Tercero' : 'Crear Tercero' }}
            </button>
